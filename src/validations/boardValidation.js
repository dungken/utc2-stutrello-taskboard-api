...
const createNew = async (req, res, next) => {
  ...
  const correctCondition = Joi.object({
    title: Joi.string().required().min(3).max(50).trim().strict().messages({
      'any.required': 'Title is required (dungken)',
      'string.empty': 'Title is not allowed to be empty (dungken)',
      'string.min': 'Title length must be at least 3 characters long (dungken)',
      'string.max': 'Title length must be less than or equal to 5 characters long (dungken)',
      'string.trim': 'Title must not have leading or trailing whitespace (dungken)'
    }),
     ...
     * Làm như trên thì sau này dù các  có thêm hay sửa gì vào cái BOARD_TYPES trong file constants thì ở những chỗ dùng Joi trong Model hay Validation cũng không cần phải đụng vào nữa. Tối ưu gọn gàng luôn.
    */
